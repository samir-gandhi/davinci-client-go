...
func TestTrigger_Configuration_RoundTrip(t *testing.T) {
	...
	testCases := map[string]struct {
		runTrigger       func(t *testing.T)
		runPolicyTrigger func(t *testing.T)
	}{
		"trigger-type-only": {
			runTrigger: func(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			if tc.runTrigger != nil {
				tc.runTrigger(t)
			}
			if tc.runPolicyTrigger != nil {
				tc.runPolicyTrigger(t)
			}
		})
	}
}
